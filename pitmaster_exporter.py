...
TEMPERATURE_EXPORT_PATH = os.path.join(
    CURRENT_SCRIPT_PATH, 'pitmaster_export.json')
...
def refresh_current_temp_gauges():
    ...
    if os.path.exists(TEMPERATURE_EXPORT_PATH):
        with open(TEMPERATURE_EXPORT_PATH, 'r') as f:
            payload = json.load(f)
            temperature_celsius = payload['current_temperature_celsius']
    else:
        print(f"\tDEBUG: {TEMPERATURE_EXPORT_PATH} does not exist")
